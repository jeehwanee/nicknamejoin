...
public class MemberController {
	...
	@ResponseBody //join.jsp에 해당 메소드의 return값(fail/success)를 반환해주기 위함
	public String memberNickChkPOST(String memberNick) throws Exception {
		int result = memberservice.nickCheck(memberNick); //존재하면 0, 존재하지 않으면 1 반환
		logger.info("결과값: "+ result);
		if(result != 0) {
			return "fail"; //중복된 아이디 존재
		}
	...
	public String mailCheckGET(String email) throws Exception {
		...
		String content = //이메일 내용
				"안녕하세요." +
				"<br>" + "회원가입을 위한 인증번호는 [" + checkNum + "] 입니다." + "<br>" +
